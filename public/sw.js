...
const SHELL_CACHE = 'hts-shell-v221-approved-soft-sfx';
...
const MEDIA_CACHE = 'hts-media-v9';
    ...
    caches.keys().then((cacheNames) => Promise.all(
      cacheNames
        .filter((name) => (
          ...
          (name.startsWith('hts-media-') && name !== MEDIA_CACHE) ||
...
self.addEventListener('fetch', (event) => {
  ...
  const isMedia = ['audio', 'font', 'image'].includes(event.request.destination) ||
    /\.(?:avif|gif|ico|jpe?g|mp3|ogg|png|svg|wav|webp|woff2?)$/.test(url.pathname);
  if (isMedia) {
