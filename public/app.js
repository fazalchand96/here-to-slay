...
const LANDSCAPE_CLASS_PROGRESS_ASSETS = Object.freeze(
    Array.from({ length: 10 }, (_, index) =>
        `assets/skin/landscape-old-polish/class-progress-${index}of9.webp`
    )
);
...
function updatePremiumBoardBackground(actionPoints, classProgress = 0) {
    ...
    board.style.removeProperty('--landscape-ap-module');
    board.style.removeProperty('--landscape-class-module');
}
...
function buildBoardParts(data, ctx) {
    ...
    const classProgressAsset = LANDSCAPE_CLASS_PROGRESS_ASSETS[classProgress];
    const winTrackHtml = document.body.classList.contains('portrait')
        ...
        : `<img class="landscape-class-progress-image"
            src="${classProgressAsset}"
            alt="Class progress: ${classProgress} of 9"
            title="Collect 9 different classes to win"
            draggable="false">`;
