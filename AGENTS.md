...
## Consumption-efficient workflow

- Keep each task narrowly scoped with explicit acceptance criteria. Inspect only
  the files and history needed for that task.
- Use one agent by default. Only use parallel agents, web research, browser
  automation, screenshots, or full simulations when the task specifically
  requires them.
- Run the most relevant individual test file while iterating. Run
  `npm test` once after the implementation is stable.
- Keep command output concise: prefer summaries and targeted searches over
  dumping entire logs or large generated files into the task context.
- Continue related work in the same task, but start a fresh task for an
  unrelated feature so stale context does not accumulate.
