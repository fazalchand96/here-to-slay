...
test('Serious Grey destroys an opponent Hero with one mobile target tap', async ({ browser }) => {
    const errors = [];
    ...
    await host.screenshot({
        path: path.join(process.cwd(), 'output', 'browser-qa', 'serious-grey-target-v203-874x402.png')
    });

    // This is intentionally one tap: no hidden second confirmation is allowed.
